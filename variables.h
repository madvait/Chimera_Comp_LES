...
typedef struct UserCtx {
  ...
  DM cda;       // Data structure for compressible 5 variables 
  DMDALocalInfo info;

  Vec   Q,lQ,Q_sum;   // vec for compressible vars
  Vec   Qold,lQold;   // vec for compressible vars

  /* Vec	Cent;	// Coordinates of cell centers */
  /* Vec   Centx,Centy,Centz; */
  Vec 	Csi, Eta, Zet, Aj,Sigma,lSigma;
  ...
  PetscReal     Ma;     // Mach number
  PetscReal     gamma;  // ideal gas cp/cv=1.4 
  PetscReal     Pr;     // Prandtle number

  // added for new poisson solver
  PetscReal     FluxIntpSum,flux,ratio;
  ...
  //PetscBool rstart_flg; // Advait 
  PetscInt _this, block_number, ti, tiout;//, tistart;
  PetscInt *idx_from;
  ...
  Vec	lNu_t, lF1,Nu_t;		// eddy viscosity, f1 constant for SST model
  Vec	lCs,Cs;
