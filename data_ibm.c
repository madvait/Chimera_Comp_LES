					  ...
					  PetscInt Add_dUndt)
{
  ...
  PetscInt i,j,k, isothermal=0;
 
  PetscReal  cv1, cv2, cv3, gamma=1.4;
  //  Cmpnts   ***ucat, ***lucat;
  ...
  current = user->ibmlist[ibi].head;
	...
	wall_function_loglaw(user, 1.e-16, sc, sb, Ua, Uc, &Ub, 
...
/*       } */
	...
	//  Tb=1.0;	// Check with Amir
	q[k][j][i].rho=p[k][j][i]*(user->gamma*user->Ma*user->Ma)/Tb;
	//p[k][j][i]=q[k][j][i].rho/(user->gamma*user->Ma*user->Ma)*Tb;

         //PetscPrintf(PETSC_COMM_WORLD,"ibi nvert  %d        %le\n",ibi,nvert[k][j][i]);	 
	...
	p[k][j][i] += q[k][j][i].rho * (Ua_n - Ua_nold) / user->dt * (sc-sb);
      ...
      q[k][j][i].rhoE = p[k][j][i]/(gamma-1) + 0.5*(q[k][j][i].rhoU*q[k][j][i].rhoU+
