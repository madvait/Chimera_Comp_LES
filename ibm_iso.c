...
#include "stdlib.h"
#include "time.h"
extern PetscInt thin, block_number,invicid, sediment, isothermal;
...
/*             happens if ibi==0--not anymore! set nvert=0 manually!*/
{
  ...
  PetscInt rank, flg=0;
  PetscOptionsGetInt(NULL, NULL, "-search_flg", &flg, NULL);
  ...
  for (k=zs; k<ze; k++) {
    for (j=ys; j<ye; j++) {
      for (i=xs; i<xe; i++) { //changed xs and xe to lxs and lxe for the ramp
  ...
  if (flg) PetscPrintf(PETSC_COMM_WORLD, "ray tracing done!!!\n");
  ...
  PetscPrintf(PETSC_COMM_WORLD, "IBM thin  %d %d %le %le %le %le %le %le\n", ibm->n_v, ibm->n_elmt, xbp_max, xbp_min, ybp_max, ybp_min, zbp_max, zbp_min);
    PetscInt cutthrough;
  if(isothermal==2){          // Isothermal 2 for ramp
    for (k=zs; k<ze; k++) {
      for (j=ys; j<ye; j++) {
	for (i=xs; i<xe; i++) {
	  if (coor[k][j][i].x > xbp_min && coor[k][j][i].x < xbp_max &&
  ...
  if(isothermal==1){ // Isothermal 1 for dP/dn ==0
    for (k=lzs; k<lze; k++) {
      for (j=lys; j<lye; j++) {
	      for (i=lxs; i<lxe; i++) {
	        if (coor[k][j][i].x > xbp_min && coor[k][j][i].x < xbp_max &&
  ...
  if (flg) PetscPrintf(PETSC_COMM_WORLD, "IB node near immersed!!!\n");
  ...
  // This is to avoid islanding
  PetscInt around, tmp;
  for (tmp=0;tmp<2; tmp++) {
  if(isothermal ==2){         //Advait
    for (k=zs; k<ze; k++) {
      for (j=ys; j<ye; j++) {
	for (i=xs; i<xe; i++) {
	  if (nvert[k][j][i] <0) nvert[k][j][i] = 0;
	  ...
	  if ((int)(nvert[k][j][i]+0.5) <0.5) {
	    for (kk=km; kk<kp+1; kk++)
	      if ((int)(lnvert[kk][j][i] +0.5) >1 && kk!=k)
		around++;

	    for (jj=jm; jj<jp+1; jj++)
	      if ((int)(lnvert[k][jj][i] +0.5) >1 && jj!=j)
		around++;

	    for (ii=im; ii<ip+1; ii++)
	      if ((int)(lnvert[k][j][ii] +0.5) >1 && ii!=i)
		around++;
	  }
  ...
  if (flg) PetscPrintf(PETSC_COMM_WORLD, "IB islanding done!!!\n");
	  ...
	  if (ibm_intp.imode<0) {
	    ...
	    if (k==0 || k==mz-1) {	     
	      ...
	      ibm_intp.k3 = k;
		       ...
		       j==0 || j==my-1) {
	      if (flg) PetscPrintf(PETSC_COMM_SELF, "%%%%IBM Searching Special! %d %d %d\n", i, j, k);
	      cell = ibm_intp.cell;
	      if (ibm->nf_z[cell] > 0) {
		ptmp = coor[k+1][j][i];
		ibm_intp.d_i = sqrt((coor[k][j][i].x - ptmp.x) *
...
if(isothermal==1){
  ...
  if (flg)  PetscPrintf(PETSC_COMM_WORLD, "IB node interpolation coeff  !!!\n");
...
/*   DMDAVecGetArray(fda, user->lCent, &coor); */
  ...
  //  if (user->nvert[lidx(i-1,j,k,user)] == 0) {
    ...
    if (ibminfo->imode >=0) {
        DMDAVecRestoreArray(fda, Coor,&coor);return(0);
    }
					  ...
					  PetscInt Add_dUndt)
{
  ...
  PetscReal     a_sound, Lambda1, L1, Pa, rhoa, rhob, rhoc, rho_diff, rho_min, Tb,Tc;
  CompVars	***lq,***q, ***qold;
  PetscReal     ***ustar, Twall=1.;
  PetscInt      tmp, itr_tmp=5;// Why 5?
  Vec           coords;
  PetscInt      twoD=0, flg=0;
  PetscOptionsGetInt(NULL, NULL, "-twoD", &twoD, NULL);
  PetscOptionsGetInt(NULL, NULL, "-ibm_intp_flg", &flg, NULL);
  ...
  for (tmp=0; tmp<itr_tmp; tmp++) {
    ...
    if (isothermal) DMDAVecGetArray(cda, user->Qold,  &qold);
    ...
    while (current) {
      ...
      if (twoD==1) nfx=0.; else nfx=coor[k][j][i].x-ibminfo->pmin.x;
      if (twoD==2) nfy=0.; else nfy=coor[k][j][i].y-ibminfo->pmin.y;
      if (twoD==3) nfz=0.; else nfz=coor[k][j][i].z-ibminfo->pmin.z;
      double dr=sqrt(nfx*nfx+nfy*nfy+nfz*nfz);	


      if (dr<0.000000001) {
	//if (flg) PetscPrintf(PETSC_COMM_WORLD, "!!ibm inpt dr zero at i %d j %d k %d\n", i,j,k);
	if (ni>=0) {
	  nfx = ibm->nf_x[ni];
	  nfy = ibm->nf_y[ni];
	  nfz = ibm->nf_z[ni];
  ...
  Uc.x = (lq[kp1][jp1][ip1].rhoU/lq[kp1][jp1][ip1].rho* sk1 + lq[kp2][jp2][ip2].rhoU/lq[kp2][jp2][ip2].rho* sk2 + lq[kp3][jp3][ip3].rhoU/lq[kp3][jp3][ip3].rho * sk3);
  Uc.y = (lq[kp1][jp1][ip1].rhoV/lq[kp1][jp1][ip1].rho* sk1 + lq[kp2][jp2][ip2].rhoV/lq[kp2][jp2][ip2].rho* sk2 + lq[kp3][jp3][ip3].rhoV/lq[kp3][jp3][ip3].rho * sk3);
  Uc.z = (lq[kp1][jp1][ip1].rhoW/lq[kp1][jp1][ip1].rho* sk1 + lq[kp2][jp2][ip2].rhoW/lq[kp2][jp2][ip2].rho* sk2 + lq[kp3][jp3][ip3].rhoW/lq[kp3][jp3][ip3].rho * sk3);
				
  if(wallfunction==1) {
	wall_function(user, sc, sb, Ua, Uc, &Ub, &ustar[k][j][i],
		      nfx, nfy, nfz);
	//	       ibm->nf_x[ni], ibm->nf_y[ni], ibm->nf_z[ni]);
	  ...
	  if (ustar[k][j][i]<us_min) us_min=ustar[k][j][i];
	    ...
	    wall_function_loglaw(user, 1.e-16, sc, sb, Ua, Uc, &Ub,&ustar[k][j][i], 
			     nfx, nfy, nfz);
	//		     ibm->nf_x[ni], ibm->nf_y[ni], ibm->nf_z[ni]);
	    if (ustar[k][j][i]>us_max) us_max=ustar[k][j][i];
	    if (ustar[k][j][i]<us_min) us_min=ustar[k][j][i];
	  ...
	  freeslip (user, sc, sb, Ua, Uc, &Ub,
		    nfx, nfy, nfz); 
	//	    ibm->nf_x[ni], ibm->nf_y[ni], ibm->nf_z[ni]);
	else 
	  noslip (user, sc, sb, Ua, Uc, &Ub, 
		  nfx, nfy, nfz);
	//	  ibm->nf_x[ni], ibm->nf_y[ni], ibm->nf_z[ni]);
      }
...
/*       } */
      ...
      // isothermal T/Tref=p/rho * gamma*M^2
      if (isothermal==2){
      	/* The p & rho obtained by interpolation might not produce Tw - scale them  */
      	cv1 = lq[kp1][jp1][ip1].rho;
      	cv2 = lq[kp2][jp2][ip2].rho;
      	cv3 = lq[kp3][jp3][ip3].rho;
	
      	rhoc= (cv1 * sk1 + cv2 * sk2 + cv3 * sk3); //rho_c

      	Tc=p[k][j][i]/rhoc*(user->gamma*user->Ma*user->Ma);
      	// Tb interpolated similar to velocity
      	if (Tc<0.) Tc=0.;
      	Tb=(Tc-Twall)*sb/sc+Twall;
      	/* //lp[k][j][i]=p[k][j][i]*Tb; */
      	/* q[k][j][i].rho=p[k][j][i]/Tb*(user->gamma*user->Ma*user->Ma);//rho_b */
      	...
      	//	a_sound=sqrt(user->gamma*p[k][j][i]/q[k][j][i].rho);
	
      	a_sound=sqrt(Twall/(user->Ma*user->Ma)); // for isothermal wall
      	Lambda1= Ua_n - a_sound; //u_n-a
      	if (tmp==0) {Pa=p[k][j][i];q[k][j][i].rho=rhoc;}
      	// L1=lambda1*(dp/dn - rho a du_n/dn
      	L1=Lambda1/sc*
      	  ((p[k][j][i]-Pa) - rhoc*a_sound*
      	  ((Uc.x-Ua.x)*nfx+(Uc.y-Ua.y)*nfy+(Uc.z-Ua.z)*nfz));
        rhob=qold[k][j][i].rho - user->dt*L1/a_sound/a_sound;
       	/* if (fabs(sb-sc)<0.000001) */
        /* rhoa=rhob; */
      	/* else */
      	/* rhoa=rhob-(rhob-rhoc)/(sb-sc)*sb; */
      	/* Pa = rhoa*Twall/(user->gamma*user->Ma*user->Ma); */
      	if (fabs(q[k][j][i].rho-rhob)>rho_diff && i==1) rho_diff=fabs(q[k][j][i].rho-rhob);
      	if (rhob<rho_min && i==1) rho_min=rhob;

	
	q[k][j][i].rho=rhob;
      	p[k][j][i]=q[k][j][i].rho*Tb/(user->gamma*user->Ma*user->Ma);
	//p[k][j][i]=Pa*(1.-sb/sc)+p[k][j][i]*sb/sc;
      	Pa=p[k][j][i];
       
      	//if (q[k][j][i].rho<0) PetscPrintf(PETSC_COMM_SELF, "!!! rho<0 at i,j,k %d %d %d\n", i,j,k);

      }
      else if(isothermal == 1){ //Advait
        cv1 = lp[kp1][jp1][ip1]/ lq[kp1][jp1][ip1].rho;
      	cv2 = lp[kp2][jp2][ip2]/ lq[kp2][jp2][ip2].rho;
      	cv3 = lp[kp3][jp3][ip3]/ lq[kp3][jp3][ip3].rho; 
	      Tc=  (cv1 * sk1 + cv2 * sk2 + cv3 * sk3)*(user->gamma*user->Ma*user->Ma);
	      Ta=1.0;
        Tb=(Ta*(sc-sb)+Tc*(sb))/sc;
        Tb=1.0;	
	      q[k][j][i].rho=p[k][j][i]*(user->gamma*user->Ma*user->Ma)/Tb;
      }
      else {
	// drho/dn=0
	      cv1 = lq[kp1][jp1][ip1].rho;
	      ...
	      q[k][j][i].rho = (cv1 * sk1 + cv2 * sk2 + cv3 * sk3);
      }
	if (Add_dUndt) // dp/dn=rho*dUn/dt
	  p[k][j][i] += q[k][j][i].rho * (Ua_n - Ua_nold) / user->dt * (sc-sb);
     // else drho/dn=0
    
      q[k][j][i].rhoU = q[k][j][i].rho * Ub.x;      
      q[k][j][i].rhoV = q[k][j][i].rho * Ub.y; 
      q[k][j][i].rhoW = q[k][j][i].rho * Ub.z; 


      // rhoE from Eq of State
      //q[k][j][i].rhoE = p[k][j][i]/(gamma-1) + 0.5*(Ub.x*Ub.x+Ub.y*Ub.y+Ub.z*Ub.z)*q[k][j][i].rho;
      q[k][j][i].rhoE = p[k][j][i]/(user->gamma-1) + 
	0.5*(q[k][j][i].rhoU*q[k][j][i].rhoU+
	     q[k][j][i].rhoV*q[k][j][i].rhoV+
	     q[k][j][i].rhoW*q[k][j][i].rhoW)/q[k][j][i].rho;

      /* cv1 = lq[kp1][jp1][ip1].rhoE; */
      /* cv2 = lq[kp2][jp2][ip2].rhoE; */
      /* cv3 = lq[kp3][jp3][ip3].rhoE; */
	
      /* q[k][j][i].rhoE = (cv1 * sk1 + cv2 * sk2 + cv3 * sk3); */  


      if (flg && tmp==0) 
	PetscPrintf(PETSC_COMM_SELF, "ibm intp itr %d  ijk %d %d %d rhob %f  L1 %f, Ua_n %f p %f Uc %f sb %f sc %f \n", tmp, i,j,k, rhob,  L1, Ua_n, p[k][j][i], Uc.z, sb, sc);    
    }


    DMDAVecRestoreArray(cda, user->lQ, &lq);
    DMDAVecRestoreArray(cda, user->Q, &q);
    if (isothermal) DMDAVecRestoreArray(cda, user->Qold,  &qold);
  ...
  if (flg) {
    PetscReal rho_diffSum=0, rho_min_g=0;
    MPI_Allreduce(&rho_diff, &rho_diffSum,1,MPI_DOUBLE,MPI_SUM,PETSC_COMM_WORLD);
    MPI_Allreduce(&rho_min, &rho_min_g,1,MPI_DOUBLE,MPI_MIN,PETSC_COMM_WORLD);
    PetscPrintf(PETSC_COMM_WORLD,"linear interpolation works... rho_diff %f min %f\n", rho_diffSum, rho_min_g);
  }
  } //tmp
